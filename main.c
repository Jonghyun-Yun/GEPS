...
#pragma omp parallel for private(ind, k, l) default(shared)
        ...
        // i: item, ind: item
				SCHOOL[a].new_item_mat[ind][i] =
				  new_item_distance[ind];
...
#pragma omp parallel for private(k) default(shared)
		  for (k = 1; k <= nSCHOOL; k++) {
			...
			avg_ran[k] *= var_ran[k];
			// NOTE: update δ in Eq 7 ((log)-normal + normal model)
			oldmu[((i - 1) * (i - 2) / 2 + j)][k] =
