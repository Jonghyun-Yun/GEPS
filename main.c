...
#pragma omp parallel
  ...
  jump_Z = dvector(
      0,
      nITEM); // NOTE: z? person?, size = (nITEM - 0 + 1): tuned by count_samp,
              // 레이턴트 스페이스의 중안에 있는 애들이 점프를 잘 안해서...
  ...
  for (k = 0; k <= nSCHOOL; k++) {
    ...
    SCHOOL[k].dataset = (int **)malloc(
        sizeof(int *) *
        (nMAX + 1)); // NOTE: **dataset: item response dataset (nSAMPLE X nITEM)
    SCHOOL[k].count_samp = (int *)malloc(
        sizeof(int) *
        (nMAX +
         1)); // NOTE: (nMAX+1) array, not matrix, 학교별로 레이턴트 포지션
              // 평균구하기 위한 값, colsum or rowsum of item matrix
    SCHOOL[k].count_item = (int *)malloc(
        sizeof(int) * (nITEM + 1)); // 학교별로 레이턴트 포지션 평균구하기 위한
                                    // 값, colsum or rowsum of item matrix
    SCHOOL[k].Y = (int ***)malloc(sizeof(int **) *
                                  (nITEM + 1)); // NOTE: Yi,n×n for item i
    SCHOOL[k].U = (int ***)malloc(sizeof(int **) *
                                  (nMAX + 1)); // NOTE: Uk,p×p for respondent k
  ...
  for (v = 0; v < repeat; v++) {
    ...
    for (i = 1; i <= nITEM * (nITEM - 1) / 2; i++) {
      olddelta[i] = -1.5 + 3.0 * rand() / RAND_MAX; // U(-1.5,1.5)
      oldtau[i] = 100.0;
      for (j = 1; j <= nSCHOOL; j++)
...
#pragma omp parallel for private(ind, k, l) default(shared)
          ...
          for (j = 2; j <= nITEM; j++)
            for (k = 1; k < j; k++) {
              ...
              // oldmu[((j-1)*(j-2)/2+k)][a], log(SCHOOL[a].new_item_mat[j][k]),
