		...
		for(iter = 1; iter <= niter; iter++){
			for(a = 1; a <= nSCHOOL; a++){
				...
				for(i = 1; i <= nITEM; i++){
					...
					for(j = 1; j <= nDIM; j++){
						SCHOOL[a].new_Zitem[i][j] = SCHOOL[a].old_Zitem[i][j] + jump_mu * gasdev();
					...
					for(j = 2; j <= nITEM; j++)
						for(k = 1; k < j; k++){
							if(SCHOOL[a].new_item_mat[j][k] > 0.0001) num += dlognorm(log(SCHOOL[a].new_item_mat[j][k]), oldmu[((j-1)*(j-2)/2+k)][a], sqrt(oldsigma[a]));
							else num += dlognorm(log(0.0001), oldmu[((j-1)*(j-2)/2+k)][a], sqrt(oldsigma[a]));
							if(SCHOOL[a].old_item_mat[j][k] > 0.0001) den += dlognorm(log(SCHOOL[a].old_item_mat[j][k]), oldmu[((j-1)*(j-2)/2+k)][a], sqrt(oldsigma[a]));
							else den += dlognorm(log(0.0001), oldmu[((j-1)*(j-2)/2+k)][a], sqrt(oldsigma[a]));
					...
					for(k = 1; k <= ncount[a]; k++)
						if(SCHOOL[a].dataset[k][i] == 1)
							for(j = 1; j <= nDIM; j++){
								num += dlognorm(SCHOOL[a].old_Zsamp[k][j], SCHOOL[a].new_Zmean[k][j], sqrt(SCHOOL[a].oldsigma));
								den += dlognorm(SCHOOL[a].old_Zsamp[k][j], SCHOOL[a].old_Zmean[k][j], sqrt(SCHOOL[a].oldsigma));
							}
				...
				for(i = 2; i <= nITEM; i++)
					for(j = 1; j < i; j++){
						...
						if(SCHOOL[k].old_item_mat[i][j] > 0.0001)
							post_b += 0.5 * (log(SCHOOL[k].old_item_mat[i][j]) - oldmu[((i-1)*(i-2)/2+j)][k]) * (log(SCHOOL[k].old_item_mat[i][j]) - oldmu[((i-1)*(i-2)/2+j)][k]);
						else post_b += 0.5 * (log(0.0001) - oldmu[((i-1)*(i-2)/2+j)][k]) * (log(0.0001) - oldmu[((i-1)*(i-2)/2+j)][k]);
					}
					oldsigma[k] = 1.0 / Rgamma(post_a, post_b); // Change Array of Variables oldsigma
			}
			...
			for(i = 2; i <= nITEM; i++)
				for(j = 1; j < i; j++){
					...
					for(k = 1; k <= nSCHOOL; k++){
						post_a += 0.5;
						post_b += 0.5 * (oldmu[((i-1)*(i-2)/2+j)][k] - olddelta[((i-1)*(i-2)/2+j)]) * (oldmu[((i-1)*(i-2)/2+j)][k] - olddelta[((i-1)*(i-2)/2+j)]);
					}
					oldtau[((i-1)*(i-2)/2+j)] = 1.0 / Rgamma(post_a, post_b);

					#pragma omp parallel for private(k) default(shared)
					for(k = 1; k <= nSCHOOL; k++){
						var_ran[k] = 1.0 / (1.0 / oldtau[((i-1)*(i-2)/2+j)] + 1.0 / oldsigma[k]); // Change Array of Variable  var_ran
						avg_ran[k] = (1.0 / oldtau[((i-1)*(i-2)/2+j)]) * olddelta[((i-1)*(i-2)/2+j)];
						if(SCHOOL[k].old_item_mat[i][j] > 0.0001) avg_ran[k] += (1.0 / oldsigma[k]) * log(SCHOOL[k].old_item_mat[i][j]);
						else avg_ran[k] += (1.0 / oldsigma[k]) * log(0.0001);
						avg_ran[k] *= var_ran[k];
						oldmu[((i-1)*(i-2)/2+j)][k] = avg_ran[k] + sqrt(var_ran[k]) * gasdev();
					}

					var_fix = 1.0 / (1.0 / pr_var_delta + nSCHOOL / oldtau[((i-1)*(i-2)/2+j)]);
					avg_fix = 0.0;
					#pragma omp parallel for private(k) default(shared)
					for(k = 1; k <= nSCHOOL; k++) avg_fix += oldmu[((i-1)*(i-2)/2+j)][k] / nSCHOOL;
